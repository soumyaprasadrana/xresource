...
import org.xresource.core.validation.XValidationUtil;
...
public class XMetadataScanner {
    ...
    public void scan(String tableName, Class<?> entityClass, Class<?> repository) {
        ...
        if (entityClass.isAnnotationPresent(XResourceAuthGroup.class)
                || entityClass.isAnnotationPresent(XResourceAuthGroups.class)) {
            for (XResourceAuthGroup auth : entityClass.getAnnotationsByType(XResourceAuthGroup.class)) {
                metadata.addResourceAccess(auth.role(), auth.access());
            }
        }
        if (repository.isAnnotationPresent(XResourceAuthGroup.class)
                || repository.isAnnotationPresent(XResourceAuthGroups.class)) {
            for (XResourceAuthGroup auth : repository.getAnnotationsByType(XResourceAuthGroup.class)) {
                metadata.addResourceAccess(auth.role(), auth.access());
            }
        }
    ...
    private String resolveType(Field field) {
        ...
        log.debug("Field '%s' is of type Object.", field.getName());
        return "object";
    }
    ...
    private void processField(Field field, Class<?> entityClass, XResourceMetadata metadata,
        ...
        if (field.isAnnotationPresent(XResourceAuthGroup.class)
                || field.isAnnotationPresent(XResourceAuthGroups.class)) {
            for (XResourceAuthGroup auth : field.getAnnotationsByType(XResourceAuthGroup.class)) {
            ...
            XValidator fieldAuthResourceWriteValidator = (entity, context) -> {
                ...
                AccessLevel level = getEffectiveAccess(roles, fieldMeta);

                try {
                    field.setAccessible(true);
                    if (field.get(entity) != null && level == AccessLevel.NONE)
                        throw new XAccessDeniedException("No write access to field: " + fieldName);
